<?
	...
	if($_POST['mode']=="check_del"){
	    foreach($_POST['idx'] as $idx){
	        db_query("delete from tbl_online_community where idx='".$idx."'");
	    }
	    
	    move_page("contract_list_community.php");
	}

	if($_GET['mode']=="del"){
	    db_query("delete from tbl_online_community where idx='".$_GET['idx']."'");
	    move_page("contract_list_community.php");
	}
	...
	if($_GET['mode']=="UPDATE_SUM"){
		...
		db_query("UPDATE tbl_online_community_sum SET $fld='$value' WHERE reg_dates = '$idx' ");
	}
?>
