<?
	...
	$sql = "
		select *
		from tbl_common b
		where codes not in (1) and pcodes = ". $code ."
		order by step
	";
...
<!--
	function moveStep(act, s, e){
		...
		swapVal(eval("f.value2"+ s), eval("f.value2"+ (s+tmp1)));
	...
	function runProc(act, n, o){
		...
		case 'm2':
			if(o.value != o.defaultValue){
				f.d1.value=encodeURIComponent("act=uitem&code="+ code +"&title="+ o.value +"&gubun="+ (o.name.substr(0, 1)=="t"?"title":"value2"));
				f.target="tmpIfrm";
				f.action="rec_proc.php";
				f.submit();
			}
...
<?for($i=0; $i<$iRows; $i++){?>
		...
		<tr height="23">
			<td align="center">
				<input type="hidden" name="step<?=$i?>" id="step<?=$i?>" value="<?=$rs[$i][step]?>_<?=$rs[$i][codes]?>_<?=$rs[$i][title]?>_<?=$rs[$i][val]?>_<?=$rs[$i][val2]?>" />
			...
			<td bgcolor="#eeeeee"></td>
			<td align="center">
				<input type="text" name="value2<?=$i?>" id="value2<?=$i?>" onblur="runProc('m2', '<?=$i?>', this);" style="width:100%;" value="<?=$rs[$i][val2]?>"/>
 			</td>
			<td bgcolor="#eeeeee"></td>
			<td align="center">
