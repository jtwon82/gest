<?
	...
	$count=db_result("select count(idx) from tbl_board_content_apt where 1=1 $search_keyword");
	$sql= "select * from tbl_board_content_apt where 1=1 $search_keyword order by idx desc limit $startnum, $list_num";
	#echo $sql;
	$pRs=db_query($sql);
...
<div id="adm_wrap">
	...
	<table style="width:100%;">
		<tr>
			<td id="content_area" style="background: url() left top repeat-x;">
				<!-- 본문 -->
				<div class="content">
							...
							<tr>
								...
								<td>
									<select name="search_key" class="dSelect">
										<option value="apt_name"<?=($search_key=="apt_name" ? " selected":"")?>>apt_name</option>
										<option value="signgu"<?=($search_key=="signgu" ? " selected":"")?>>signgu</option>
									</select>
									...
									<select name="search_key2" class="dSelect">
										<option value="ym"<?=($search_key=="ym" ? " selected":"")?>>ym</option>
									</select>
									<input type="text" name="search_val2" id="search_val2" class="dInput req" title="검색어" value="<?=$search_val2?>" style="width:100px;" />
					...
					<div class="tableStyle1">
						<form name="contractaptwrite" id="contractaptwrite" method="post" action="board_pro.php" enctype="multipart/form-data">
						<input type="hidden" name="b_id" value="<?=$b_id?>" />
						<input type="hidden" name="mode" value="contractaptwrite" />
							<textarea name="contractaptlist" class="dInput" style="width:100%;height:50px;"></textarea>
							<input type="file" name="file1[]" multiple/>
							<div id="btn_top"><span class="button red"><a href="#" onclick="contractaptwrite.submit();">쓰기</a></span></div>
						</form>
					</div>
					...
					<input type="hidden" name="b_code" value="<?=$b_code?>" />
						...
						?>
							<tr>
								...
								<td><?=$sortnum?></td>
								<td style="text-align:left;padding-left:10px;"><?=$pList['signgu']?> <?=$pList['bungi']?> <?=$pList['apt_name']?> <?=$pList['size']?> <?=$pList['ym']?> <?=$pList['d']?> <?=$pList['price']?> <?=$pList['floor']?></td>
								<td><?=$pList['ym']?></td>
								<td><?=$pList['apt_name']?></td>
								<td><?=$pList['size']?></td>
								<td><?=$pList['price']?></td>
								<td><?=$pList['floor']?></td>
								<td><?=$pList['etc5']?></td>
