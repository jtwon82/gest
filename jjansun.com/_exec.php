...
/////// DB에 들어갈 값들을 정리합니다.	db_query
	...
	//LOGIN
		...
		Case "LOGIN" :
			...
			if ($_COOKIE[referer]!="" && $_COOKIE[referer]!=$ssn ){	// 친구에게 포인트지급하기
				$CHK= $_COOKIE[referer];
				...
				$tbl_member_typehistory = db_select_assoc("select count(1) cnt FROM tbl_member_typehistory
						where ssn='$ssn' and action_type='$action_type' ");
				if($tbl_member_typehistory[cnt]<1){
					charge_chance($reg_ip, $_COOKIE[referer], $CHK, 'add', $chance_type2, $chance);

					db_query("insert into tbl_member_typehistory(ssn, action_type, subdata, cnt, reg_date)
						values('$ssn', '$action_type', '', 1, now()) ON DUPLICATE KEY UPDATE update_date=now(), cnt=cnt+values(cnt) ");
				}
			}
		...
		Case "GET_COMMON_INFO":
			...
			if( $clear=='clear' || rand()%100>50){
				...
				$sql= "select rnk, reg_date, /*concat('**',right(uname,1))*/ uname, concat('010****',right(pno,4))pno
							, b.giftname win_type_str, b.codes
				from (
					select @rn := if(@keyword = ssn , @rn+1, if(@keyword := ssn, 1, 1)) rnk
						, m.*
					from (
						select a.idx ,m.uname ,m.pno ,a.reg_date ,a.ssn ,a.chk , b.data2 gamecode ,a.win_type
		...
		Case "CHARGE_CHANCE":
			if ( !isset($_SESSION[USER][LOGIN_ID]) ){
				...
				if ( $chance_type == 'eeeee' ){
					...
					if (strpos($chance_type2, 'use')>-1)$user_type=100;
				...
				if( $rs[c]=='end' ){
					...
					if ( strpos($rs[c], 'lose')>-1 ){
						...
						$sql = "insert into tbl_event_joiner(event_gubun, reg_date, reg_dates, ssn, chk, win_type, reason, mobile, reg_ip, referer, uname, pno1, share_desc )
							values('$user_type', now(), left(now(), 10), '$ssn', '$CHK', 'lose', '$reason_rs[win_img]', '$mobile', '$reg_ip', '$referer', '$uname', '$pno1', '$snsType' )";
						#db_query($sql);
						...
						$pno1	= base64_encode($pno1);
						$sql = "insert into tbl_event_joiner(event_gubun, reg_date, reg_dates, ssn, chk, win_type, reason, mobile, reg_ip, referer, uname, pno1, share_desc )
							values('$user_type', now(), left(now(), 10), '$ssn', '$CHK', '$reason', '$reason_rs[win_img]', '$mobile', '$reg_ip', '$referer', '$uname', '$pno1', '$snsType' )";
						db_query($sql);
	...
	// LIST
		Case "BOARD_LIST":
			...
			$sql = "
				select
					a.*
						, b.giftname win_type_str, b.gamename
						, case when chance_type='login' then 'add' else chance_type end chance_type
						, case when chance_type2 like 'use%' then '즉석당첨'
							when chance_type2 like 'game%' then '적립게임'
							when chance_type2 like 'eventadd%' then '추가적립'
								...
								INSERT INTO ranking_score_award( reg_dates, wk, gamecode, ssn, rank, score)
									...
									FROM (
										...
										from (
											...
											and not EXISTS ( select ssn from tbl_member m where m.ssn=a.ssn and email in('lth83@naver.com','weespk@kakao.com','') )
											group by a.ssn
