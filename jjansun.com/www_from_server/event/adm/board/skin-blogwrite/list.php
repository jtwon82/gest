							...
							<tr>
								...
								<td>
									<select name="search_key" class="dSelect">
										...
										<option value="tag"<?=($search_key=="tag" ? " selected":"")?>>tag</option>
									</select>
					...
					<input type="hidden" name="b_code" value="<?=$b_code?>" />
						...
						?>
							<tr>
								...
								<td><a href="board_list.php?<?=$param?>&search_key=tag&search_val=<?=$pList['tag']?>"><?=$pList['tag']?></a></td>
								<td style="text-align:left;padding-left:10px;"><?=$pList['title']?></td>
								<td><?=$pList['link']?></td>
								<td><?=$pList['etc1']?></td>
								<td>
									<span class="button"><a href="board_write.php?mode=modify<?=$param?>">수정</a></span>
									<span class="button"><a href="board_pro.php?mode=del<?=$param?>&idx=<?=$pList['idx']?>" onclick="return really()">삭제</a></span>
								</td>
							</tr>
						<?
								$i++;
