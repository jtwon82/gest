...
@Service
public class ParserS20CoKr implements Parser {
	private static final Logger logger = LoggerFactory.getLogger(ParserS20CoKr.class);
	
	@Autowired
	private ContentDao			contentDao;
	
	@Bean
	public ParserS20CoKr getParser() {
		return new ParserS20CoKr();
	}
...
//					Elements gift_info = subPage.select(item.get("dom_gift_info"));
					...
					e.put("landing", String.format("%s", e.get("landing_uri")));
