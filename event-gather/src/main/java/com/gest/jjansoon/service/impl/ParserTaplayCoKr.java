...
@Service
public class ParserTaplayCoKr implements Parser {
	private static final Logger logger = LoggerFactory.getLogger(ParserTaplayCoKr.class);
	
	@Autowired
	private ContentDao			contentDao;
	
	@Bean
	public ParserTaplayCoKr getParser() {
		return new ParserTaplayCoKr();
	}
	
	@Override
	public void parse(Map<String, String> idx)  {
		try {
			logger.debug("{}", idx);
...
//					Elements gift_info = subPage.select(item.get("dom_gift_info"));
					...
					e.put("landing", String.format("http://%s/event/%s", e.get("domain"), e.get("landing_uri")));
